...
Simplifying using `A()` for anchors and `a()` for attributes.

```r
mapping <- list(Amne = A(natural_key_column_names,
                         attr_mne = a(column_name, hist = "historize_column_name", knot = "knot_mne"),
                         ...),
                ...)
```
...
Model any data, your model with just one anchor, model can non-destructivly grow over time so you can think about more entities later!  
