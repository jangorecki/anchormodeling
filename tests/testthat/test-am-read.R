...
test_that("AM read method", {
    ...
    expect_identical(names(am$read()), c("code", "name", "class", "mne", "desc", "obj", "hist", "knot"), info = "Defined *non-run* columns exists")
    am$run()
    am$load(mapping = list(AC = list("code", NAM = "name")),
            data = data.table(code = "1", name = "Mike"),
            meta = 1L)
    expect_identical(names(am$read()), c("code", "name", "class", "mne", "desc", "obj", "hist", "knot", "anchor", "parents", "childs"), info = "Defined *run* columns exists")
